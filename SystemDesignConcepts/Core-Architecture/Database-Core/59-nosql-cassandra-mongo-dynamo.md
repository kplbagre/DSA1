...
**Core principle:** All three are NoSQL (no rigid schema, horizontal write scaling, eventual consistency by default) but they differ on three axes: query flexibility, operational ownership, and consistency tuning model.
...
**The key insight is:** Cassandra gives you tunable consistency and raw write scale. DynamoDB gives you zero operations overhead with a constrained query model. MongoDB gives you query flexibility with per-document ACID transactions — at the cost of cross-partition scalability at extreme write volumes.
...
AXIS 3: CONSISTENCY TUNING
Strict ──────────────────────────────────────────▶ Eventually Consistent
Cassandra          MongoDB              DynamoDB
(tunable:          (configurable        (eventual by default;
 W + R > RF        read/write concern;  strongly consistent
 = strong;          multi-doc ACID       reads cost extra)
 W=1,R=1           transactions)
 = eventual)
...
WRITE PATH (per node):
  Request arrives
      │
      ▼
  Commit Log (WAL — durable write before anything)  ← durability guarantee
      │
      ▼
...
KEY INVARIANT:
  ...
  Range queries on clustering columns WITHIN a partition = fast (sorted on disk).
...
**What is a GSI?** A Global Secondary Index (GSI) is an alternate index on a DynamoDB table that allows querying by non-primary key attributes. It projects a copy of the data, so it has its own separate read/write throughput and is eventually consistent by default.
...
| **Transactions** | No multi-row ACID | Single-item atomic; multi-item via TransactWrite (up to 100 items) |
...
// MongoDB is WRONG here: 10M orders/day requiring 100K writes/sec
...
// This is ACID at MongoDB level — unavailable in Cassandra
...
- **Shopify** (MongoDB): Product catalog for 2M+ merchants, each with completely different catalog structures (electronics vs clothing vs food). MongoDB's flexible document schema means no schema migrations when a new category type is onboarded. Rich aggregation pipeline powers merchant analytics dashboards. Atlas (managed MongoDB) keeps ops lightweight.
...
| Multi-region active-active writes (no single primary) | You need multi-row ACID transactions |
...
|---|---|
| Schema is genuinely flexible (different fields per document type) | You need 100K+ writes/sec consistently (Cassandra beats it here) |
...
| You want a balance: NoSQL flexibility + some RDBMS guarantees | Strong consistency everywhere (read from primary replica cost) |
...
| **MongoDB: You lose** | Primary-only writes (single primary → vertical write scale limit); $lookup (join) is expensive and not distributed; schema flexibility becomes a liability at large team sizes |
| **MongoDB: Failure mode** | Using MongoDB for a write-heavy IoT pipeline (10M events/sec) — primary bottlenecks; Cassandra's append-only model handles this with no contention |
...
> Yes — mathematically. Quorum = floor(3/2) + 1 = 2. W=2 means the write is acknowledged by 2 nodes before returning. R=2 means a read queries 2 nodes and takes the newest timestamp. With W=2 and R=2, the write set and read set must overlap by at least 1 node (W + R = 4 > RF = 3). That overlap node has the latest version — the read always includes it. The caveat: if a node in the quorum write set was unreachable and didn't receive the write, that node could serve a read with W + R > RF still satisfied via a different node pair. This is the "hinted handoff" problem — the unreachable node gets the write as a hint and applies it when it rejoins. During the outage window, reads from that node could be stale.
