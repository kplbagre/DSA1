...
**Full form:** Saga / Distributed Transaction Pattern

**Simple analogy:** You're coordinating a multi-step vacation: book flight → book hotel → book rental car. If the rental car is unavailable, you cancel the hotel AND flight (compensate). A saga is a **multi-step distributed transaction with compensations** — if any step fails, you undo all previous steps in reverse order.

**Core principle:** In microservices, traditional ACID transactions don't work (no central database). A saga replaces ACID with **compensating transactions**: each step completes independently; if one fails, previous steps are explicitly undone. There are two patterns:
- **Orchestration:** A central coordinator (saga orchestrator) controls the flow step-by-step.
- **Choreography:** Services listen to events and react autonomously (event-driven); no central coordinator.

**Why it matters in system design:** Enables multi-service operations (order → inventory → payment → shipping) with rollback guarantees, despite no distributed locking. Becomes critical at scale with 100s of microservices.
...
```
ORCHESTRATION APPROACH (Central State Machine):
┌────────────────────────────────────────────────────┐
│ Saga Orchestrator (Central Coordinator)            │
│ State Machine: PENDING → INVENTORY → PAYMENT →    │
│               DELIVERY → COMPLETED                 │
└─────┬──────────────┬──────────────────┬───────────┘
...
FAILURE + COMPENSATION:
  If Delivery fails:
    Orchestrator: Compensate Payment (refund)
    Orchestrator: Compensate Inventory (unreserve)
        ...
        Inventory  Payment
         Services  Services
            ...
            └─ Publishes "PaymentProcessed"
                       ...
                       ▼
                  Delivery Service
                   (auto-scheduled if both events arrived)

KEY INVARIANT:
   Orchestration = centralized control (simple, single point of failure)
   Choreography = decentralized autonomy (complex, hard to debug)
   Both require idempotent steps & compensation logic
```

---

## 🎨 Visual — Saga Orchestration vs Choreography (Component Detail)
