...
6. **Check `SystemDesignConcepts/Patterns/tools-glossary.md`** when writing or editing notes that name specific tools (Redis commands, Kafka internals, APNs, FCM, PostGIS, Temporal, AWS Step Functions, Cassandra, DLQ, etc.). This file is the single source of truth for plain-English definitions of all infrastructure tools used across Pattern and concept notes. If a tool is named in the note but not yet in the glossary, add it there first, then reference it. **Purpose:** ensures any named tool has a definition the reader can look up without leaving the knowledge base, and prevents duplicate inline definitions scattered across multiple files.
...
| `52-numbers-to-know-scale-triggers.md` | Numbers to Know & Scale Triggers (2026 hardware baselines, back-of-envelope formula, scale trigger thresholds, common anti-patterns) | ✅ Done |
| `53-webhooks.md` | Webhooks (HTTP push callbacks, HMAC-SHA256 signature verification, idempotency key pattern, replay attack prevention, endpoint registration, DocuSign Connect) | ✅ Done |
| `54-redis-internals.md` | Redis Internals — Atomicity, Persistence & the Right Command (single-threaded event loop, five atomic weapons: DECR/SET NX EX/Lua/EXPIRE/ZADD, RDB vs AOF persistence with fsync modes, eviction policies, problem→weapon mapping table for 8 prepared problems) | ✅ Done |
...
| Jul 4, 2026 | **`tools-glossary.md` registered in Mandatory Pre-Work (step 6).** File lives at `SystemDesignConcepts/Patterns/tools-glossary.md`. Contains plain-English definitions + key commands for 27 tools used across Pattern and concept notes (Redis Pub/Sub, ZSET, INCR, SETNX, MULTI/EXEC, Lua scripts, GEOADD, TTL; Kafka topic/partition/offset/consumer group/retention; APNs, FCM, DLQ, PostGIS, Geohash, Cassandra, Temporal, AWS Step Functions, ICE/STUN/TURN, WebRTC). Created because these tools were referenced by name across 9+ Pattern notes without any definition, leaving a gap for readers who don't already know the tools. Rule: if a note names a tool not yet in the glossary, add it there first, then reference it. |
| Jul 9, 2026 | **52 + 53 registered; Pub/Sub section + Webhooks concept added.** `52-numbers-to-know-scale-triggers.md` was on disk (created Jul 2) but absent from AGENTS.md tracked table — now registered (was already counted in INDEX.md). `53-webhooks.md` created: HMAC-SHA256 verification, idempotency key pattern, replay attack prevention, endpoint registration, async processing, DocuSign Connect context. Pub/Sub Pattern section added to `19-message-queues-kafka-rabbitmq.md` (FanoutExchange code, point-to-point vs pub/sub ASCII diagram, SNS+SQS comparison, Redis Pub/Sub caveat). HMAC added to `tools-glossary.md`. Webhook + pub/sub resources added to `resources.md`. Total: 55 tracked notes (54 main + 4 advanced companions → 55 main + 4 advanced companions). |
| Jul 9, 2026 | **`54-redis-internals.md` created.** Foundation-level Redis reference covering: single-threaded event loop (structural source of atomicity), five atomic weapons with Java implementations (DECR inventory reservation, SET NX EX distributed lock, Lua script check-and-claim, EXPIRE idempotency/tokens, ZADD sliding window + delayed queue), RDB vs AOF persistence trade-offs with all fsync modes, eviction policy selection (allkeys-lru vs volatile-lru), problem→weapon mapping table for 8 prepared problems. Resources and AGENTS.md tracking updated in same step. Prompted by AOF coverage gap in `42-inventory-management-booking.md` Tier 2 Q&A. |
