...
Fix: **`InheritableThreadLocal` does NOT work for thread pool workers** — pool threads are created once at startup and reused across requests; they inherit context only at creation time, not per-submitted-task. The correct approach: capture the shadow context value before submitting the task and restore it inside the `Runnable`/`Callable` (snapshot-and-restore pattern), or use the Alibaba `TransmittableThreadLocal` (TTL) library which automates context propagation for pooled threads. If using reactive code, pass the shadow flag through the reactive context (`Mono.contextWrite`) rather than a thread-local. Verify this propagation explicitly — a unit test that runs shadow through a `CompletableFuture.supplyAsync()` will expose the failure if `InheritableThreadLocal` is used.
...
| Jul 20, 2026 | Fixed HIGH-severity error introduced in Jul 11 audit: "InheritableThreadLocal for thread pools" does NOT work — pool threads are created once and reused, so they never inherit per-request context. Corrected to: snapshot-and-restore pattern (capture value before submission, restore in Runnable) or Alibaba TransmittableThreadLocal library. |
