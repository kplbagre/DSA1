...
> Dual write is for migrating data between two DBs during a service extraction — you control both DBs. The outbox pattern (see `07-cdc-outbox.md`) is for reliable event publishing to Kafka — you write both the business row and an outbox row into the same DB in a single ACID transaction (those two rows are atomic because they share the same DB). A separate CDC relay (Debezium watching the outbox table) or polling publisher then reads the outbox and publishes to Kafka afterward — at-least-once. The Kafka write is never inside the DB transaction; you cannot enroll Kafka in a relational DB transaction. Different problem: dual write handles DB-to-DB migration; outbox handles DB-to-message-queue reliability.
...
| Jul 20, 2026 | Fixed HIGH-severity error in Q&A: "outbox pattern = write DB row + Kafka event in same transaction" — that is impossible; Kafka cannot participate in a relational DB transaction. Corrected to: outbox writes two DB rows (business + outbox) atomically; CDC/polling relay publishes outbox row to Kafka separately (at-least-once). |
