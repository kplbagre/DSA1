...
**REPEATABLE_READ** — a transaction sees a consistent snapshot as it was when the transaction started. Any reads of the same row within the transaction always return the same value, even if other transactions commit changes.

**SERIALIZABLE** — the strongest isolation level. Transactions execute as if they were serial (one after another), with no interleaving. The database prevents not only non-repeatable reads but also **phantom reads** (when a new row matching a query appears because another transaction inserted it after your initial read).

**The key connection (and a common myth to avoid):** Optimistic locking does **NOT** require REPEATABLE_READ — it works correctly at READ_COMMITTED, which is the default in PostgreSQL, Oracle, and SQL Server. That is the whole point of the version column: the atomic `UPDATE ... WHERE version = N` is evaluated against *committed* data at write time, so a concurrent update that bumped the version is detected as a 0-row update regardless of what snapshot you read earlier. The version CAS **is** the lost-update prevention mechanism — you use it precisely so you *don't* need a higher isolation level. (If anything, it's a *naive* read-modify-write with no version column that benefits from higher isolation.) Pessimistic locking (`SELECT ... FOR UPDATE`) serializes access to **the specific rows it locks** — it does not, by itself, prevent phantom inserts of new rows matching a predicate, so calling it "SERIALIZABLE" is an overstatement.

**Phantom reads and gap locks:** A phantom read occurs when you query rows matching a condition (e.g., `SELECT * FROM orders WHERE price > 100`), and another transaction inserts a row matching that condition. On a re-read, a new row appears. `SELECT ... FOR UPDATE` locks only the rows that already exist and match — it cannot lock a row that isn't there yet. To prevent phantoms you need either SERIALIZABLE isolation or, in InnoDB's REPEATABLE_READ, **gap locks** (locks on index gaps between rows) that block inserts into the queried range. The database applies these automatically.

**In an interview, if asked:** "Optimistic locking with a version column is correct at READ_COMMITTED — the atomic conditional UPDATE detects conflicts at write time, so it doesn't depend on a higher isolation level. Pessimistic `SELECT FOR UPDATE` serializes the specific rows it locks, but doesn't prevent predicate phantoms without gap locks or SERIALIZABLE. The choice between the two is about conflict frequency, not isolation level."
...
### Q (Tier 2): "Does optimistic locking still work if the database runs at READ_COMMITTED (the common default)?"
> Yes — and this is the point most people get backwards. Optimistic locking works **correctly** at READ_COMMITTED; it does not require REPEATABLE_READ. Here's why: you read `version=7`, do your work, then issue `UPDATE ... SET version=8, ... WHERE id=? AND version=7`. The database evaluates that `WHERE version=7` against the *latest committed* data at write time. If another transaction already bumped it to 8, your UPDATE matches **0 rows** — you detect the conflict and retry. The stale snapshot you read earlier is irrelevant; the atomic conditional write is what guarantees safety. So the version CAS *prevents* lost updates at READ_COMMITTED rather than causing them. (A naive read-modify-write with **no** version column is the thing that needs higher isolation — the version column is precisely the technique that lets you avoid it.) Note: Hibernate's `@Version` does **not** change your transaction isolation level — it simply appends `AND version=?` to the UPDATE and checks the affected-row count; isolation stays whatever your connection/pool/transaction config sets.
...
| Jul 19, 2026 | **Major factual correction (inverted concept).** The Isolation-Levels section and its Tier-2 Q&A repeatedly claimed optimistic locking *requires/assumes* REPEATABLE_READ and produces lost updates at READ_COMMITTED — this is backwards. Rewrote it: optimistic locking (version-column CAS) is correct at READ_COMMITTED because the atomic `UPDATE ... WHERE version=N` detects conflicts at write time; the version check *is* the lost-update prevention, so it doesn't need higher isolation. Also removed the false claim that Hibernate `@Version` auto-sets the isolation level, and softened "`SELECT FOR UPDATE` = SERIALIZABLE" (it serializes only the specific locked rows; no predicate-phantom protection without gap locks). |
