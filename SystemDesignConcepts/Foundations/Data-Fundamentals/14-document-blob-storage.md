...
| **You lose** | Two-system consistency (S3 upload + DB write must both succeed — partial failures leave orphaned objects; this is the real risk, not storage-layer consistency), S3 is not queryable (you must maintain a metadata DB separately). **Note: S3 is strongly read-after-write consistent for all operations since Dec 2020** — a PUT then immediate GET on a new key will NOT return a stale/404 result. Do not cite S3 eventual consistency in 2026; the only consistency gap you own is the S3-vs-DB dual write. |
...
| Jul 19, 2026 | **Factual fix (outdated).** The trade-off table described S3 as eventually consistent ("PUT then GET may return 404") — S3 has been **strongly read-after-write consistent for all operations since Dec 2020**. Corrected the claim and clarified that the only consistency risk you own is the S3-vs-DB dual write, not the storage layer. |
