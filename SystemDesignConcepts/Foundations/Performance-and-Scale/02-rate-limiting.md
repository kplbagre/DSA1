...
**Full form:** Rate Limiter / Request Throttling

**Simple analogy:** A nightclub bouncer controls who enters: only 10 people per hour allowed. Once the limit hits, new arrivals are turned away (or queued), even if they look legitimate. Rate limiting protects your backend from being overwhelmed.

**Core principle:** Rate limiting controls how many requests a client can make in a time window (e.g., 100 requests per minute). Requests beyond the limit are rejected or queued. Protects the backend from overload, prevents abuse, and ensures fair resource sharing among multiple clients.

**Why it matters in system design:** Without rate limiting, one misbehaving client (or one bug) can saturate your entire service, causing timeouts for legitimate users. It's the first line of defense for any public API.

---

## 🎨 Visual — System Topology: Rate Limiting in Architecture

```
EXTERNAL CLIENTS (varied traffic patterns)
    │
    ├─ Client A: normal, steady requests
    ├─ Client B: burst traffic
    └─ Client C: potential attacker (DDoS)
             │
             ▼
┌────────────────────────────────┐
│ Rate Limiter (Gateway/Proxy)   │
│                                │
│ ┌──────────────────────────────┤
│ │ Per-client quota tracking    │
│ │ - Client A: 50/100 req/min   │
│ │ - Client B: 100/100 req/min  │
│ │ - Client C: 99/100 req/min   │
│ │ (all near limit → careful)   │
│ └──────────────────────────────┤
│                                │
│ ┌──────────────────────────────┤
│ │ Decision logic               │
│ │ - Token bucket OR            │
│ │ - Sliding window counter OR  │
│ │ - Leaky bucket               │
│ └──────────────────────────────┤
│                                │
└────────┬──────────┬──────────┬─┘
    ...
    Allow ✅    Allow ✅    Reject ❌
         │          │          │
         ▼          ▼          ▼
    Backend    Backend    429 Too Many
    Service    Service    Requests

KEY INVARIANT:
   Each client has a quota (limit per time window)
   Requests beyond quota are rejected or queued
...
## 🎨 Visual — Rate Limiting Algorithms: Token Bucket vs Sliding Window (Component Detail)
