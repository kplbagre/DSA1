...
> Each dimension has its own limit: user (1,000 req/min), IP (10,000 req/min), API key (100,000 req/min). When the bot makes requests from 100 different IPs, each IP gets 100 req/min (100 requests × 100 IPs = 10,000, hitting the IP-dimension limit). But the API key limit is 100,000 req/min, so the bot is caught by the IP dimension. If the bot tries more IPs to spread further, they'd all hit the IP limit. To detect this pattern, I also monitor for high cardinality: if one API key is used from > 50 distinct IPs in 1 minute, that's suspicious — I further reduce that API key's limit or require additional verification.
