...
**Full form:** Cache / In-Memory Cache

**Simple analogy:** You're a busy executive who needs to look up information frequently. Instead of walking to the filing cabinet (database) every time (takes 30 seconds), you hire an assistant who sits next to you with the most-asked-for answers on a notepad (cache). Next time someone asks, the assistant answers in 1 second from the notepad.

**Core principle:** A cache stores frequently-accessed data in fast, expensive memory (RAM) rather than slow, cheap storage (disk/database). On a read, check cache first (hit = fast). If not cached (miss), fetch from database, store in cache, and return. This reduces load on the database and speeds up reads 10–100x.

**Why it matters in system design:** Without caching, a single database node handles ~10,000 reads/sec. With caching (Redis), you can handle 100,000+ reads/sec. Every read-heavy system (user profiles, feeds, search) relies on caching.

---

## 🎨 Visual — System Topology: Caching in Architecture

```
READ REQUESTS (users fetching data)
    │
    ▼
┌──────────────────────────────────┐
│ Application Server               │
...
└────────────┬─────────────────────┘
             ...
             ▼
      ┌──────────────┐
      │ Cache (Redis)│   ← Fast (1ms)
      │ Hot data     │
      │ (frequently  │
      │  accessed)   │
      └──────┬───────┘
             │
       Cache HIT? (99% of reads)
             │
        ┌────┴─────┐
        │           │
       YES ✅      NO (MISS) ❌
...
┌──────────────────────────┐
│ Browser Cache            │ (10 sec, client-side)
├──────────────────────────┤
│ CDN Cache                │ (10 min, edge locations)
├──────────────────────────┤
│ Application Cache (Redis)│ (1 hour, app layer)
├──────────────────────────┤
│ Database Query Cache     │ (in-process, DB level)
└──────────────────────────┘

KEY INVARIANT:
   Cache hits reduce load on database
   Cache misses must be filled from database
   TTL (time-to-live) controls staleness
   Cache stampede occurs when many requests miss same key simultaneously
```

---

## 🎨 Visual — Cache Strategies: Cache-Aside vs Write-Through (Component Detail)
